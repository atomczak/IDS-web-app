...
#         self.csvwriter = csv.DictWriter(self.file, ['guid','result','sentence'])
...
# messages = {
#     'sentence': 'Given an instance with %(applicability)s we expect %(requirements)s. %(name)s (%(guid)s) has %(applicability_ans)s so is not compliant',
#     'entity': 'an entity %(name)s',
#     'type': 'of predefined type %(value)s'
# }
...
if __name__ == "__main__":
    ...
    # c=1
    # for issue in report.statements:
    #     print(str(c) + ') ' + str(issue))
    #     c+=1

    # TODO add other handlers (CSV, HTML, etc.)
