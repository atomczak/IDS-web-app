...
def create_page():
    if request.method == "POST":
        ...
        # TODO add user registry (number of users   )
...
def delete(id):
    try:
        for i in range(len(session["ids_db"]["specifications"])):
            session["ids_db"]["specifications"] = list(
                filter(lambda i: i["id"] != id, session["ids_db"]["specifications"])
            )
        # renumber:
        for i in range(len(session["ids_db"]["specifications"])):
...
def add_requirement(id):
    ...
    val = ""  # my %s..." % type
    if type == "entity":
        session["ids_db"]["specifications"][sid]["applicability"][0]["value"].append(
            val
        )
    else:
        req = {
            "id": len(session["ids_db"]["specifications"][sid][part]) + 1,
            "type": type,
            "value": val,
        }
        if type == "property":
...
def delete_requirement(id):
    ...
    try:
        if type == "entity":
            if (
                len(
                    session["ids_db"]["specifications"][sid]["applicability"][0][
                        "value"
                    ]
                )
                > 1
            ):
                del session["ids_db"]["specifications"][sid]["applicability"][0][
                    "value"
                ][-1]
            else:
            ...
            for i in range(len(session["ids_db"]["specifications"][sid][part])):
                session["ids_db"]["specifications"][sid][part] = list(
                    filter(
                        lambda i: i["id"] != rid,
                        session["ids_db"]["specifications"][sid][part],
                    )
                )
            # renumber:
            for i in range(len(session["ids_db"]["specifications"][sid][part])):
...
@app.route(
    "/api/specification", methods=["POST", "GET"]
)  # &<specId>&<type>&<reqId>&<fieldValue>/")
def updateSpecification():  # specId, reqId, type, fieldValue
    data = request.json["id"].split(".")
    session["ids_db"]["specifications"][int(data[1]) - 1][data[2]][int(data[3]) - 1][
        data[4]
    ] = request.json["value"]
    session.modified = True
    return "success"
...
@app.route(
    "/api/entity", methods=["POST", "GET"]
)  # &<specId>&<type>&<reqId>&<fieldValue>/")
def updateEntity():  # specId, reqId, type, fieldValue
    data = request.json["id"].split(".")
    session["ids_db"]["specifications"][int(data[1]) - 1][data[2]][int(data[3]) - 1][
        "value"
    ][int(data[4]) - 1] = request.json["value"]
    session.modified = True
    return "success"
...
def downloadFile():
    filename = os.path.join(
        app.static_folder, "custom_ids", "User_" + request.remote_addr + ".xml"
    )
    try:
        ...
        return send_file(
            filename,
            mimetype="text/xml",
            download_name=date.today().isoformat()
            + "_IDS_"
            + session["ids_db"]["name"]
            + ".xml",
            as_attachment=True,
...
def convertToIds(dict, filename):
    ...
    i = ids.ids(
        ifcversion=None
        if dict["ifcversion"] == "Any"
        else dict["ifcversion"]
        if dict["ifcversion"]
        else None,
    ...
    # TODO make it better, less loops
        ...
        for req in spec["applicability"]:
            if req["type"] == "entity":
                for entity in req["value"]:
                    ...
                    i.specifications.append(
                        ids.specification(name=f"{spec['name']} ({entity})")
                    )
                    ...
                    for req in spec["applicability"]:
                        if req["type"] == "predefined":
                            predefined = req["value"]
                    if predefined:
                        e = ids.entity.create(name=entity, predefinedtype=predefined)
                    else:
                        e = ids.entity.create(name=entity)
                    i.specifications[-1].add_applicability(e)

                    # classification
                    for req in spec["applicability"]:
                        r = None
                        if req["type"] == "classification":
                            r = ids.classification.create(
                                location="any", value=req["value"], system=req["system"]
                            )
                        if r:
                    ...
                    for req in spec["applicability"]:
                        ...
                        if req["type"] == "property":
                            r = ids.property.create(
                                location="any",
                                propertyset=req["pset"],
                                name=req["property"],
                                value=req["value"],
                            )
                        if r:
                    ...
                    # in requirements order is not important
                    for req in spec["requirements"]:
                        ...
                        if req["type"] == "classification":
                            ...
                            r = ids.entity.create(
                                name=entity, predefinedtype=req["value"]
                            )
                        if r:
...
# S.declaration_by_name("IfcBuildingElement").subtypes()
# [decl.name() for decl in S.declarations()]

# IFC_ENTITIES = [...]

# subtypes of IfcProduct:
#   "IfcElement"
#   "IfcGrid"
# subtypes of IfcElement:
#   "IfcBuildingElement"
#   "IfcDistributionElement" >> "IfcDistributionFlowElement" >> "IfcFlowFitting" >> "IfcDuctFitting"
# subtypes of IfcBuildingElement:
#   "IfcBeam"
#   "IfcBuildingElementProxy"
#   "IfcBuildingElementComponent" # only in IFC2x3!
#   "IfcChimney" # only in IFC4!
#   "IfcColumn"
#   "IfcCovering"
#   "IfcCurtainWall"
#   "IfcDoor"
    ...
    "specifications": [
        ...
        },
        {
            ...
            "applicability": [
                ...
                {"id": 5, "type": "material", "value": "TEST"},
            ],
            "requirements": [
                {"id": 1, "type": "predefined", "value": "TEST"},
                {"id": 2, "type": "classification", "value": "TEST", "system": "TEST"},
                {
                    "id": 3,
                    "type": "property",
                    "value": "TEST",
